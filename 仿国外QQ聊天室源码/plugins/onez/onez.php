...
ONEZ;
  ...
  function sitehash(){
    ...
    $time=time();
    return base64_encode("$appid\t$siteid\t$sitekey\t$time\t".$G['this']->token."\t".md5($appkey));
  }
  function post($post){
    global $G;
    $item=$G['this']->option();
    ...
    if($post['format']=='url'){
      return 'http://www.yunziyuan.com/api/usersite.php?'.http_build_query($post);
    }
    $data=onez()->post('http://www.yunziyuan.com/api/usersite.php',http_build_query($post));
    if($post['format']=='json'){
