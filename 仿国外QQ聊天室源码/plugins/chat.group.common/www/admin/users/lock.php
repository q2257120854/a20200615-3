...
$type=onez()->gp('type');
define('CUR_URL',onez('chat.group.common')->href('/admin/users/lock.php',0));
...
$record=$G['this']->data()->open('group_users')->page("key1='lock' order by id desc");
?>
      ...
      <table class="table table-striped">
        ...
        <tbody class="todo-list">
          ...
          <tr>
            ...
            <td>
              <?php echo onez('chat.group.common')->nickname($rs['lock_masteruid'])?>
            </td>
            <td>
              <?php 
if($rs['token']=='guest'){
  ...
  echo onez('chat.group.common')->nickname($rs['userid']);
            ...
            <td>
              <?php echo $rs['lock_because']?>
            </td>
            <td>
              <?php echo date('Y-m-d H:i:s',$rs['lock_exptime'])?>
            </td>
            <td>
...
onez.del=function(id){
  onez.confirm('您确定要解除此用户的禁言状态吗？',function(){
