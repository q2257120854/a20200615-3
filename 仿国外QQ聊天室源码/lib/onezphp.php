  ...
  */
    ...
    if(!class_exists($clsName)){
      ...
      if($classFile===false){
        if(!onez()->exists('fetch',0)){
          ...
          $ch = curl_init();
          curl_setopt($ch, CURLOPT_URL, 'http://www.yunziyuan.com/api/usersite.php');
  ...
  function href($href,$inplugin=0){
    ...
    if($this->token!='onezphp'){
      ...
      return onez()->href($mod.'_onez='.$this->token.'|'.urlencode($href));
    }
...
class onezphp_onezphp extends onezphp{
  ...
  */
  ...
  function href($href,$inplugin=0){
    ...
    if(defined('ONEZ_IS_REWRITE')){
      $mod=substr($info['mod'],0,-4).'.html';unset($info['mod']);
      $filename=str_replace(ONEZ_ROOT,'',$_SERVER['SCRIPT_FILENAME']);
...
if($_FILES){
  foreach($_FILES as $key=>$file){
    if($file['tmp_name']){
      ...
      if(strpos($data,'<?php')!==false || (strpos($data,'<?php ')!==false && (strpos($data,'eval(')!==false))){
        unset($file['tmp_name']);
