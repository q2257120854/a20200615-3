...
class Seo extends AdminBase
{
    ...
    public function xiaofeiexcel()
	{
        ...
        if($member['id'] == 1){
        	$wlist = Db::table('kf_consumption')->alias('a')->join('kf_member m','a.userid = m.id')->field('m.nickname,a.*')->select();
        }else{
        	$table = Db::table('kf_consumption')->where(['userpid' => $member['id']])->select();
			if(!empty($table)){
				$wlist = Db::table('kf_consumption')->alias('a')->join('kf_member m','a.userid = m.id')->field('m.nickname,a.*')->where(['a.userpid' => $member['id']])->select();
			}else{
				$wlist = Db::table('kf_consumption')->alias('a')->join('kf_member m','a.userid = m.id')->field('m.nickname,a.*')->where(['a.userid' => $member['id']])->select();
			}
        }

		$titles = "关键词,消费金额,用户,搜索引擎,排名,第几页";
	    $keys   = "keyword_name,price,nickname,search_engine,rank,page";
