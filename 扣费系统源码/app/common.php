 ...
 * 获取数据POST数据
 */
function getKeywordPost($data){
    $result = [];
    foreach($data as $key=>$value){
        $result[$value['search_engine']][] = $value;
    }
    //   dump($result);
    $retultData=[];
    foreach($result as $k=>$v){
        $utlarr=[];
        foreach($v as $k1=>$v1){
            $utlarr[$v1['url']][]=['keywords'=>$v1['keywords']];
        }
        $retultData[$k]=$utlarr;
    }
    //dump($retultData);
    //=====================开始循环组装POST数据
    $retData=[];
    foreach($retultData as $k2=>$v2){
        foreach($v2 as $k3=>$v3){
            $ksywordArr=[];
            foreach($v3 as $k4=>$v4){
                $ksywordArr[]=$v4['keywords'];
            }
            $retData[]=['search_engine'=>$k2,'url'=>$k3,'keywords'=>implode('|',$ksywordArr),'notify_url'=>config('api_domain')."/api.php/seo/getrank",'app_key'=>config('TD_APPKEY')];
        }
