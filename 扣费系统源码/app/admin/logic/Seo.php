...
class Seo extends AdminBase
{
     ...
     * ,count(c.id) as knum
     */
    public function getWebList($where = [], $field = 'm.*,b.username,', $order = '', $paginate = DB_LIST_ROWS)
    {
...
//  		$order = 'm.create_time desc'
     ...
     * 获取关键词列表
     */
    public function getKeywordList($where = [], $order = '',$paginate = DB_LIST_ROWS){

    	$order = 'm.create_time desc';
		$field = 'm.*,b.username,c.url';
     ...
     * 获取消费记录表
     */
    public function getXiaofeiList($where = [], $field = 'm.*,b.username,c.url,ff.dllirun_one,ff.ptlirun_one,ff.dllirun_two,ff.ptlirun_two', $order = 'create_time desc', $paginate = DB_LIST_ROWS){
    ...
    public function getWebListWhere($data = [])
    {
        ...
        $where = [];
		//根据用户id查询他所有的网站
        !empty($data['userid']) && $where['m.userid'] = tp_decrypt($data['userid']);
		//未优化的站点status是0和关键词为0的
		isset($data['status']) && $where['m.status'] = 0;
	

        !empty($data['search_data']) && $where['m.web_name|b.username|m.url'] = ['like', '%'.$data['search_data'].'%'];
		//代理下的网站
		!empty($data['dl_search']) && $where['m.userpid'] = ['eq',tp_decrypt($data['dl_search'])];
        $info=session('member_info');
        if($info['leader_id']==1){
        	//上级id是1有两种情况,一种是注册会员的上级id就是1,一种是代理
        	$access = Db('auth_group_access')->where(['member_id' => $info['id']])->find();
			if($access['group_id'] == 19){
				$where['m.userid'] = ['eq',$info['id']];
			}else{
				$where['b.leader_id']=['eq',$info['id']];
			}
    ...
    public function getKeywordListWhere($data = []){
        ...
        //待审核的任务
        !empty($data['shenhe']) && $where['m.jk_status'] = [['lt',1],['gt','-2']];
        //审核不通过任务
        !empty($data['shenhebtg']) && $where['m.jk_status'] = ['eq','-2'];
		//申请停止优化的任务
		!empty($data['tzshyouhua']) && $where['m.youhua'] = [['lt',1],['gt','-2']];
		//已停止优化的任务
		!empty($data['tzyouhua']) && $where['m.youhua'] = ['eq','-2'];
		//申请报错的任务
		!empty($data['applyerror']) && $where['m.applyerror'] = ['eq',0];
		//拒绝停止优化的任务
		!empty($data['jjshyouhua']) && $where['m.youhua'] = ['eq','-3'];
		
		//已经处理的报错任务
		!empty($data['applysuccess']) && $where['m.applyerror'] = ['eq','2'];
        !empty($data['search_data']) && $where['c.url'] = ['like', '%'.$data['search_data'].'%'];
    ...
    public function webstatus($where = []){
        ...
        try{
           ...
           // $key=$this->modelKeywords->where("web_id={$where['id']}")->update(['jk_status'=>$status]);
            $key=$this->modelKeywords->where("web_id={$where['id']}")->update(['youhua'=>$youhua]);
            // 提交事务
            Db::commit();
        } catch (\Exception $e) {
            // 回滚事务
        ...
        if($where['status']==1){
            return [RESULT_SUCCESS, '修改状态成功，网站下关键词全部停止优化', $url];
        }else{
            return [RESULT_SUCCESS, '修改状态成功，网站下关键词全部开始优化', $url];
        }
    ...
    public function keyWordAdd($data = [] ,$jump = 'keywordlist'){
		...
		//判断用户等级,
		$member_info = Db('member')->where(['id' => $userid])->find();
		//判断代理是否设置等级
        ...
        foreach($data['keyword_data']['keyword'] as $k=>$v){
			...
			$ptlirun_one = $pageone - $cbone;//平台利润第一页
			$ptlirun_two = $pagetow - $cbtow;//平台利润第二页
			if($member_info['rank'] == 0){
				$dllirun_one = 0;
				$dllirun_two = 0;
			}else{
				$discount = Db('discount')->where(['user_id' => $member_info['leader_id']])->order('rank_id asc')->select();
			
				if(empty($discount[$member_info['rank'] - 1])){
					$dllirun_one = 0;
					$dllirun_two = 0;
				}else{
					if($pageone == 0){
						...
						$dllirun_one = $pageone * $discount[$member_info['rank'] - 1]['discount'] / 100 - $pageone;
					}
					if($pagetow == 0){
						$dllirun_two = 0;
					}else{
						$dllirun_two = $pagetow * $discount[$member_info['rank'] - 1]['discount'] / 100 - $pagetow;
					}
				}
			}
            foreach($data['search_engine'] as $k3=>$v3) {
                ...
                $insertData[] = ['userid' => $userid, 'userpid' => $info['leader_id'], 'web_id' => $web_id, 'create_time' => time(), 'keywords' => $v, 'page_one' => $pageone, 'page_tow' => $pagetow, 'page_one_cb' => $cbone, 'page_tow_cb' => $cbtow, 'search_engine' => $v3, 'jk_status' => $data['jk_status'],'ptlirun_one' => $ptlirun_one,'ptlirun_two' => $ptlirun_two,'dllirun_one' => $dllirun_one,'dllirun_two' => $dllirun_two, 'youhua' => $data['youhua']];
            }
            $keyarr[]=$v;//-----------------------关键词数组
        }
...
//		if($data['jk_status'] == 1){$data['youhua'] = 1;}
    ...
    public function keywordAllEdit($data=[]){
		...
		if($data['youhua'] == '-3'){
			if($data['jujue'] == ''){
				return [RESULT_ERROR,'请填写拒绝停止优化原因!'];
			}
		}
		if($data['jk_status'] == '-2'){
			if($data['youhua'] == '' || $data['youhua'] == '-2' || $data['youhua'] == 2){
				$data['youhua'] = 2;
			}else{
				return [RESULT_ERROR,'审核不通过的关键词,不能进行优化!'];
			}
			if($data['beizhu'] == ''){
				return [RESULT_ERROR,'请填写审核不通过原因'];
			}
		}
		if($data['jk_status'] == '1'){$data['youhua'] = 1;}
		if($data['jk_status'] == '-2'){
        ...
        if(MEMBER_ID == 1){
            if(trim($data['page_one'])=='' && trim($data['page_tow'])=='' && trim($data['page_one_cb'])=='' && trim($data['page_tow_cb'])=='' && trim($data['jk_status'])=='' && trim($data['youhua']) == ''){
		...
		if(trim($data['jujue']) != ''){$upInfo['jujue'] = trim($data['jujue']);}
		if(trim($data['beizhu']) != ''){$upInfo['beizhu'] = trim($data['beizhu']);}
        if(trim($data['jk_status'])!=''){$upInfo['jk_status']=trim($data['jk_status']);}
        if(trim($data['youhua']) != ''){$upInfo['youhua'] = trim($data['youhua']);}
    ...
    public function chongzhiAdd($data=[]){
      ...
      /*  if($info['leader_id']<=1){
       ...
       //---------------判断是否有权限给用户充值
		...
		//获取会员信息
		if($userinfo['leader_id'] == 1){
			$memberinfo = Db('member')->where(['id' => $userinfo['id']])->find();
			if($memberinfo['price'] < $data['price']){
				return [RESULT_ERROR,'充值金额大于你的余额,不能进行充值!'];
			}
    ...
    public function xiaofeiexcel($where = [])
	{
	    ...
	    foreach($wlist as $k => $v){
            $wlist[$k]['create_time'] = date("Y-m-d H:i:s", $v['create_time']);
        }
	
		$titles = "关键词,网站地址,消费金额,用户,搜索引擎,排名,第几页,时间";
	    $keys   = "keyword_name,url,price,nickname,search_engine,rank,page,create_time";
   ...
   //关键词列表导出
    public function excel()
	{
		...
		$member_id = $member['id'];
        if($member['id'] == 1){
        	$wlist = Db::table('kf_keywords')->alias('a')
        	->join('kf_weblist w','a.web_id = w.id', 'LEFT')
        	->join('kf_member m','a.userid = m.id', 'LEFT')
        	->select();
			...
			if(!empty($table)){
				...
				->join('kf_weblist w','a.web_id = w.id', 'LEFT')
				->join('kf_member m','a.userid = m.id', 'LEFT')
				->join('kf_consumption d', 'a.id = d.keyword_id', 'LEFT')
        		->field('a.*,w.url,m.nickname,count(d.id)')
				->where("a.userpid = $member_id")
				->select();
			}else{
				$wlist = Db::table('kf_keywords')->alias('a')
				->join('kf_weblist w','a.web_id = w.id', 'LEFT')
		...
		if(!empty($wlist)){
			foreach($wlist as $k => $v){
				$wlist[$k]['dabiao'] = Db('consumption')->where(['keyword_id' => $v['id']])->count();
			}
		}
	    $titles = "关键词,所属域名,引擎搜索,所属用户,第一页价格,第二页价格,当前排名,达标天数";
	    $keys   = "keywords,url,search_engine,nickname,page_one,page_tow,rank,dabiao";
	    action_log('导出', '导出关键词列表');   
...
//		print_r($data)
	...
	{
			$url = url('seo/keywordlist');
			if($data['applyerrbeizhu'] == ''){return [RESULT_ERROR,'请填写报错原因!'];}
			
			if(!isset($data['applyerrorchuli'])){
				if($data['applyerror'] == 0){return [RESULT_ERROR,'该报错正在处理中,请耐心等待结果!'];}
				$result = Db('keywords')->where(['id' => $data['id']])->update(['applyerror' => 0,'applyerrbeizhu' => $data['applyerrbeizhu']]);
				return $result ? [RESULT_SUCCESS,'报错提交成功,请耐心等待处理结果!',$url . '?applyerror=1'] : [RESULT_ERROR,'提交失败,请刷新页面重新提交!'];
			}else{
			
				$result = Db('keywords')->where(['id' => $data['id']])->update(['applyerror' => $data['applyerrorchuli'],'applyerrbeizhu' => $data['applyerrbeizhu']]);
				return $result ? [RESULT_SUCCESS,'报错处理成功',$url . '?applysuccess=1'] : [RESULT_ERROR,'处理失败,请刷新页面重新提交!'];
			}
