...
if ($my == "deletebyid") {
	$getid = $_GET['id'];
                  $row = $DB->get_row("select * from ku_kamilog where id =".$getid." limit 1");
                  if(!$row){
                      @header('Content-Type: text/html; charset=UTF-8');
                      exit("<script language='javascript'>alert('发生异常！');history.go(-1);</script>");
                  }
                  $sql = "delete from ku_kamilog where id = ".$getid;
                  if($DB->query($sql)){ 
                    @header('Content-Type: text/html; charset=UTF-8');
                    exit("<script language='javascript'>alert('删除卡密日志成功！');window.location.href='./carminelog.html';</script>");
                }else{
                    @header('Content-Type: text/html; charset=UTF-8');
                    exit("<script language='javascript'>alert('删除卡密日志失败！');history.go(-1);</script>");
                }
}
if($my == "detarz"){ 
          
                 $sql = " truncate table ku_kamilog";
                if($DB->query($sql)){
                    @header('Content-Type: text/html; charset=UTF-8');
                    exit("<script language='javascript'>alert('成功删除所有卡密日志记录！');window.location.href='./carminelog.html';</script>");
                }else{
                 
                    @header('Content-Type: text/html; charset=UTF-8');
                    exit("<script language='javascript'>alert('删除所有卡密日志失败！');history.go(-1);</script>");
                }
            }
...
if(!empty($_SESSION['pz']) && $_SESSION['pz'] != ""){
    $pz = $_SESSION['pz'];   
    $numrows=$DB->count("SELECT count(*) from ku_kamilog where operator  like '%$pz%' or out_trade_no like '%$pz%' or quantity like '%$pz%' ");
}else{

    $numrows=$DB->count("SELECT count(*) from ku_kamilog");
}
...
if(!empty($_SESSION['pz']) && $_SESSION['pz'] != ""){
    $pz = $_SESSION['pz'];
    $sql = "SELECT * FROM ku_kamilog WHERE  operator  like '%$pz%' or out_trade_no like '%$pz%' or quantity like '%$pz%'  order by id desc limit $offset,$pagesize";
}else{
    $sql = "SELECT * FROM ku_kamilog  order by id desc limit $offset,$pagesize";
...
<div class="white-box">
     ...
     ?> 
                        ...
                        <a href="?my=detarz" onclick="return confirm('你确定要删除全部卡密日志吗？');">
  ...
  <div class="row">              
            <div class="col-md-12">
                 <div class="white-box">
                      <h2 class="header-title">卡密生成记录</h2>
                 ...
                 ?>
                      <tr>
                        ...
                        <td>
                            ...
                            <a href="<?=$res['download']?>" title="查看卡密"><i class="icon-paper-clip"></i>
                          </a> 
