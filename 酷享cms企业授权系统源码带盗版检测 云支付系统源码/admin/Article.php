...
if ($my == "deleteid") {
    $id = $_GET['id'];
                  $row = $DB->get_row("select * from ku_article where id =".$id." limit 1");
                  if(!$row){
                      @header('Content-Type: text/html; charset=UTF-8');
                      exit("<script language='javascript'>alert('删除文章异常！');history.go(-1);</script>");
                  }
                  $sql = "delete from ku_article where id = ".$id;
                  if($DB->query($sql)){
                    @header('Content-Type: text/html; charset=UTF-8');
                    exit("<script language='javascript'>alert('删除文章成功！');window.location.href='./Article.html';</script>");
                }else{
                    @header('Content-Type: text/html; charset=UTF-8');
                    exit("<script language='javascript'>alert('删除文章失败！');history.go(-1);</script>");
                }
}?>
<div class="white-box">
                <div class="row">
                 ...
                 while($res = $DB->fetch($rs))
                  {
                   ?>
                                    <tr>
                                        <td><?=$res['title']?></td>
                                        <td><?=mb_substr($res['content'], 0, 10, 'utf-8')?></td>
                                        <td><?=$res['addtime']?></td>
                                        <td><img src="<?=$res['img']?>" width="80px" height="35px"/></td>
                                        <td><?=Article_type($res['type'])?></td>
                                        <td><?=Article_state($res['state'])?></td>
                                        <td><?=$res['oper']?></td>
                                        <td width=10%;>
                                          <a href="./Articleeide.html?my=edit&id=<?=$res['id']?>" class="btn btn-xs btn-purple" title="编辑">
                                           编辑
                                              </a>
