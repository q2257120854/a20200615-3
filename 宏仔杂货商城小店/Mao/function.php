...
function dx($sjh,$type,$yzm,$name,$kdgs,$ydh){
    if($type == 1){//验证码
        $querys = "content=【宏仔杂货商城小店】验证码为：[{$yzm}]，欢迎登陆平台！&mobile={$sjh}";
    }elseif($type == 2){//发货
        $querys = "content=【宏仔杂货商城小店】[{$name}]您好，您的货物已由[{$kdgs}]快递公司发出，单号[{$ydh}]，请注意查收。&mobile={$sjh}";
    }elseif($type == 3){//新订单
        $querys = "content=【宏仔杂货商城小店】已接收到新的订单，请登陆后台查看。&mobile={$sjh}";
    }elseif($type == 4){//新工单
        $querys = "content=【宏仔杂货商城小店】已接收到新的工单，请登陆后台处理。&mobile={$sjh}";
    }elseif($type == 5){//工单处理
        $querys = "content=【宏仔杂货商城小店】您所提交的工单已被客服处理，请登陆至工单列表查 看详情。&mobile={$sjh}";
    }

    $host = "https://cxkjsms.market.alicloudapi.com";
    $path = "/chuangxinsms/dxjk";
    $method = "POST";
    $appcode = "123456";//阿里云接口AppSecret
    $headers = array();
