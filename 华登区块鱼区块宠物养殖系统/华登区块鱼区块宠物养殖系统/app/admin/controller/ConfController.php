...
class ConfController extends AdminCheckAuth
{
    ...
    public function webConf()
    {
        ...
        $webConf = [
            ...
            'openRedisQueue' => empty($conf['openRedisQueue']) ? '0' : $conf['openRedisQueue'],
            'iosDownloadlink' => empty($conf['iosDownloadlink']) ? '' : $conf['iosDownloadlink'],
            'androidDownloadlink' => empty($conf['androidDownloadlink']) ? '' : $conf['androidDownloadlink'],
            'customer_service' => empty($conf['customer_service']) ? '' : $conf['customer_service'],
        ];
        ...
        if($this->request->isPost()){
            ...
            if ($result !== true) {
                ...
                $conf = "<?php return [
                    ...
                    'code'=>'{$param["code"]}',
                    'domain'=>'{$param["domain"]}',
                    'port_cache_time'=>'{$param["port_cache_time"]}',
                    'openRedisQueue'=>'{$param["openRedisQueue"]}',
                    'iosDownloadlink'=>'{$param["iosDownloadlink"]}',
                    'androidDownloadlink'=>'{$param["androidDownloadlink"]}',
    ...
    public function dayuConf()
    {
        ...
        $dayuConf = [
            ...
            'default_account'     => empty($conf['default_account']) ? '' : $conf['default_account'],
            'default_password' => empty($conf['default_password']) ? '' : $conf['default_password'],
            'default_sign' => empty($conf['default_sign']) ? '' : $conf['default_sign'],
            'sms_type' => empty($conf['sms_type']) ? '0' : $conf['sms_type']
        ];
    ...
    public function dayuConfSubmit()
    {
        if($this->request->isPost()){
            ...
            if ($result !== true) {
                ...
                $conf = "<?php return ['appkey'=>'{$param["appkey"]}','secretkey'=>'{$param["secretkey"]}','FreeSignName'=>'{$param["FreeSignName"]}','default_account'=>'{$param["default_account"]}','default_password'=>'{$param["default_password"]}','default_sign'=>'{$param["default_sign"]}','sms_type'=>'{$param["sms_type"]}'];";
    ...
    public function userConf()
    {
        ...
        $userConf = [
            'registration_agreement'     => empty($conf['registration_agreement']) ? '' : $conf['registration_agreement'],
        ];
        return view('', ['userConf'=>$userConf]);
    }

    //提交用户配置
    public function userConfSubmit()
    {
        if($this->request->isPost()){
            $param = vae_get_param();
            $result = $this->validate($param, 'app\admin\validate\Conf.userConf');
            if ($result !== true) {
                return vae_assign(0,$result);
            } else {
                $conf = "<?php return ['registration_agreement'=>'{$param["registration_agreement"]}'];";
                file_put_contents(VAE_ROOT . "data/conf/extra/userconfig.php",$conf);
                return vae_assign();
            }
        }
    }
