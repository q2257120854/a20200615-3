...
var ZYFILE = {
    ...
    $.fn.zyUpload = function(options, param) {
     ...
     //               var filename=file.name
					...
					// html += '	<textarea  style="resize:none"  class="form-control">'+filename +'</textarea>';
                    html += "</div>"
                } else {
                    html += '<div id="uploadList_' + file.index + '" class="upload_append_list">';
                    html += '	<div class="file_bar">';
...
var response = eval('(' + response + ')'); 
                       ...
                       // $("#uploadList_" + file.index).attr("data-img",response);
                        // 上传完成把图片插入div元素
var yuansuhtml='<div class="col-md-3 col-sm-4 col-xs-6 meiyouuimmag">';
yuansuhtml+='<button type="button" class="btn btn btn-sm btn-danger" onclick="qichu(this)">删除</button>'
yuansuhtml+='<div><img src="'+response.imgurl+'" ></div>';
yuansuhtml+='<textarea data-img="'+response.img+'" style="resize:none" class="form-control" rows="3">'+response.text+'</textarea></div>';
$('#imagasdasd').append(yuansuhtml);
