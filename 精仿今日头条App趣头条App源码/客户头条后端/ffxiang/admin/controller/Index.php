...
class Index extends Common
{
  ...
  public function videoUploads(){
    if(request()->file('video')){
          require_once APP_PATH . '/../vendor/qiniu/autoload.php';
          $db=db::name('member_config')->where(['type'=>'sdkQiniu'])->select();
          $accessKey = $db['0']['text'];
          $secretKey = $db['1']['text'];
          $auth = new Auth($accessKey, $secretKey);
          $bucket = $db['2']['text'];
...
// $rets="1231";
  ...
  {   
      ...
      $yaoqing=Db::name('yaoqing')->field(['uid','jinbi'])->select();
      foreach ($yaoqing as  $v) {
        $userid=$v['uid'];
        $where['uid']=$v['uid'];
        $tijilu=Db::name('yaoqing_records')->where($where)->select();
        if($v['jinbi']>=100 && !$tijilu){
          $yushu=($v['jinbi']%100);
          $tijin=($v['jinbi']-$yushu)/100;
          $jinbi=$v['jinbi']-$yushu;
      ...
      if ($status) {
        return ['ret'=>'全部用户金币转零钱成功'];
      }else{
        return ['ret'=>'执行过程中出错，请稍后再试'];
      }
