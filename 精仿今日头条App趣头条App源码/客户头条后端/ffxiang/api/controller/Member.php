   ...
   if($yaoqing[0]['jinbi']>=100 && !$tijilu)
        {
                ...
                $jinbi=$yaoqing[0]['jinbi']-$yushu;
                 ...
                 $result = Db::execute('update fx_yaoqing set money_num =money_num+'.$tijin.' where uid = '.$userid.' ');
	...
	//创建贴吧
	public function tiebaEdit(){
		$term_id=input('term_id/d');
		...
		$slogan=input('post.slogan','','strip_tags') ? input('post.slogan','','strip_tags') : '';
		...
		$count=Db::name('typeid')->where(['uid'=>$this->member['id']])->count();
		if($count>=$config['tieba_limit']){
			exit(json_encode(['status'=>0,'error'=>'已达到创建贴吧数']));
		}
		
		$db=Db::name('typeid')->where(['title'=>$title])->value('uid');
		if($db){
			exit(json_encode(['status'=>0,'error'=>'不能创建重复贴吧']));
		}
		
		$data['uid'] = $this->member['id'];
		$data['image'] = $logo;
		$data['tid'] = get_tieba_rootid();
		
		//自动审核
		if($config['tieba_switch']){
			 $data['hide']=1; 
		}else{
			$data['hide']=0; 
		}
		$data['title'] = $title;
		$data['keywords'] = $slogan;
		...
		$id=Db::name('typeid')->insertGetId($data);
		if($id){
			if($data['hide']==0){
				_records($this->member['id'],'创建贴吧ID：'.$id.'等待审核');
                $data=['status'=>1,'msg'=>'贴吧创建成功，等待审核'];
			}else{
				_records($this->member['id'],'创建贴吧ID：'.$id.'自动审核通过');
                $data=['status'=>1,'msg'=>'贴吧创建成功'];
				$ret = _tieba_jiangli($this->member,$id);
				if($ret['status'] == 1){
					$data['tips'] = $ret;
				}else{
					$data['tips1'] = $ret;
			...
			Db::name('typeid')->where(['id'=>$id])->setInc('faved');
			
			
			
		}else{
			$data=['status'=>0,'error'=>'贴吧创建失败,请稍后再试'];
		}
		...
		//编辑贴吧
		    ...
		    $data['content'] = $content;
			$data['update_time']=$time;
			$db=Db::name('typeid')->where('id',$term_id)->update($data);
			if($db){
			   $data=['status'=>1,'msg'=>'编辑成功'];
			}else{
				$data=['status'=>1,'msg'=>'编辑失败或无任何修改'];
			}
		}
		exit(json_encode($data));
	}
	
	public function toBazhu(){
		$id=input('id/d');
		$tieba=Db::name('typeid')->where('id',$id)->find();
		if(!$tieba){
			$data=['status'=>0,'msg'=>'贴吧不存在'];
			exit(json_encode($data));
		}
		
		$user = Db::name('member')->where('id',$this->member['id'])->find();
		if(!$user['mobile']){
			$data=['status'=>0,'msg'=>'请先完成手机绑定'];
			exit(json_encode($data));
		}
	    /*
		if(!$user['alizh'] || !$user['wxname']){
			$data=['status'=>0,'msg'=>'请先完成支付账号或微信收款名称绑定'];
			exit(json_encode($data));
		}
		*/
		//$data['bazhu_id'] = $this->member['id'];
		//$db=Db::name('typeid')->where('id',$id)->update($data);
		$where['uid']=$this->member['id'];
        $where['aid']=$id;
		$where['type']=3;
        $where['time']=time();
		$db=Db::name('member_favorite')->insert($where);
		if($db){
			$data=['status'=>1,'msg'=>'申请成功,请等待审核'];
			exit(json_encode($data));
		}else{
			$data=['status'=>0,'msg'=>'申请失败'];
			exit(json_encode($data));
		}
	...
	public function pingDel(){
		$id=input('id/d');
		if(!$id){
			exit(json_encode(['status'=>0,'error'=>'该回复已不存在']));
		}
		$db=Db::name('article_ping')->where('id',$id)->delete();
		if($db){
	...
	//编辑帖子
	public function tieziEdit(){
		$id=input('id/d');
		$action=input('action');
		$config = config()['appConfig'];
		if(!$id){
			exit(json_encode(['status'=>0,'error'=>'该帖子已不存在']));
		}
		
		if($action == 'delete'){
			$db=Db::name('article')->where('id',$id)->delete();
			if($db){
				exit(json_encode(['status'=>1,'msg'=>'删除成功']));
			}
		}
		if($action == 'zhiding'){
			$count=Db::name('article')->where(['id'=>$id,'zhiding'=>1])->count();
			if($count >= 4){
				exit(json_encode(['status'=>0,'error'=>'已达到最大的置顶数量']));
			}
			
			$db=Db::name('article')->where('id',$id)->update(['zhiding'=>$config['tieba_zhiding_switch']]);
			if($db){
				$msg = '置顶成功';
				if($config['tieba_zhiding_switch'] == 2){
					$msg = '置顶成功,请耐心等待审核';
				}
				exit(json_encode(['status'=>1,'msg'=>$msg]));
			}
		}
		if($action == 'cancel_zhiding'){
			$db=Db::name('article')->where('id',$id)->update(['zhiding'=>0]);
			if($db){
				exit(json_encode(['status'=>1,'msg'=>'取消置顶成功']));
			}
		}
		
		exit(json_encode(['status'=>0,'error'=>'操作失败']));
    ...
    public function articleAdd(){
		...
		$articleHide = 0;
		$posttype=input('post.posttype') ? input('post.posttype') : 'weitoutiao';
		if($posttype == 'weitoutiao'){
			$articleHide = $config['weitoutiao_switch'];
		}
		if($posttype == 'tieba'){
			$articleHide = $config['tiezi_switch'];
		}
		if($articleHide){
            $where['hide']=1; 
        }else{
        ...
        $mychannel=input('post.mychannel/d') ? input('post.mychannel/d') : '';
        ...
        if(!$aritlceId){
            ...
            if($where['hide']==1){
				$tips = 0;
				if($posttype == 'tieba'){
					$tips = _tiezi_jiangli($this->member,$id);
			    }else{
                // 检测金额奖励
                if($articleMoney){
                    // 检测是否已奖励，防止重复奖励
	...
	//贴吧关注
	public function tiebaFaved(){
		$aid=input('post.id') ? input('post.id/d') : '';
		$shou=Db::name('member_favorite')->where(['uid'=>$this->member['id'],'aid'=>$aid,'type'=>2])->value('id');
		if($shou){
			$db=Db::name('member_favorite')->where(['uid'=>$this->member['id'],'aid'=>$aid,'type'=>2])->delete();
			 if($db){
			Db::name('typeid')->where(['id'=>$aid])->setDec('faved');
            _records($this->member['id'],'删除关注贴吧ID：'.$aid);
            $data=['status'=>1,'ret'=>'取消关注'];
         }else{
            $data=['status'=>0,'err'=>'取消失败'];
         }
		}else{
			$where['uid']=$this->member['id'];
            $where['aid']=$aid;
            ...
            $where['time']=time();
			 $db=Db::name('member_favorite')->insert($where);
             if($db){
				Db::name('typeid')->where(['id'=>$aid])->setInc('faved');
               _records($this->member['id'],'关注贴吧ID：'.$aid);
                $data=['status'=>1,'ret'=>'已关注'];
            }else{
                $data=['status'=>1,'err'=>'关注失败'];
            }
		}
		echo json_encode($data); 
	}
    ...
    public function fangke(){
        ...
        // var_dump($where);exit();
        $data['total']=Db::name('member_fangke')->where($where)->count();
        $data['per_page']=10;
        $data['current_page']=$pages+1;
        $data['data']=Db::name('member_fangke')->where($where)->order('id desc')->limit(10*($pages-1),10)->select();
        ...
        foreach ($data['data'] as $k => $v) {
            ...
            $data['data'][$k]['time']="访问时间：".date('m-d H:i:s',$v['time']);
            $data['data'][$k]['guanzhu']=Db::name('member_guanzhu')->where(['uid'=>$this->member['id'],'gz_uid'=>$user['id']])->value('id');
        }
        exit(json_encode($data));
    ...
    public function fangkejilu(){
        ...
        $where['f_uid']=$f_uid;
        $where['time']=time();
           $yifang=db::name('member_fangke')->where($where)->find();
           if($yifang)
           {
             exit();
           }
    ...
    public function ping(){
        ...
        if($count>100){
            ...
            if(!$pingId){
                ...
                if($db){
					$tips = 0;
					$name = Db::name('article')->where(['id'=>$aid])->find();
					if($name['weitoutiao'] == 3 && $name['hide'] == 1){
						$tips = _huitie_jiangli($this->member,$aid);
					}
                    Db::name('article')->where(['id'=>$aid])->setInc('pingNum');
					Db::name('article')->where(['id'=>$aid])->update(['update_time'=>time()]);
                    exit(json_encode(['ret'=>$db,'tips'=>$tips]));
                }else{
                    exit(json_encode(['err'=>$db]));
                }
            }else{
