<?php
 global $_W, $_GPC;
        ...
        if($op=='qb'){//全部
            ...
            $order = pdo_fetchall("select * from ".tablename($this->modulename."_wphtjorder")." where weid='{$_W['uniacid']}' {$ddorwehre} and openid='{$fans['openid']}' order by id desc");
//             echo "<pre>";
//             print_r($order);
//             exit;
            foreach($order as $k=>$v){
                ...
                $tkorder = pdo_fetch("select * from ".tablename($this->modulename."_wphorder")." where weid='{$_W['uniacid']}' and orderSn='{$v['orderid']}'  order by id desc");
                $tkordersum = pdo_fetchcolumn("select sum(commission) from ".tablename("tiger_newhu_wphorder")." where weid='{$_W['uniacid']}' and orderSn='{$v['orderid']}' and orderSubStatusName!='已失效'");
                 
                 if(!empty($tkorder)){
 
                      if($v['sh']==0 || $v['sh']==3 and $tkorder['orderSubStatusName']=='已付款'){
                          pdo_update ( $this->modulename . "_wphtjorder", array('sh'=>3,'yongjin'=>$tkorder['commission']), array ('id' => $v['id']));
                       }
                       if($tkorder['orderSubStatusName']=='已结算' and $v['sh']==3){
                         //echo $v['orderid'];
                         pdo_update ( $this->modulename . "_wphtjorder", array('sh'=>1,'yongjin'=>$tkorder['commission']), array ('id' => $v['id']));
                       }
                       if($tkorder['orderSubStatusName']=='已失效' ){
                         pdo_update ( $this->modulename . "_wphtjorder", array('sh'=>4,'yongjin'=>$tkorder['commission']), array ('id' => $v['id']));
                       }

                        if($tkorder['orderSubStatusName']=='已结算'){
                            if($v['sh']==2){
                              continue;
                            }
                            //echo $v['sh'];
                                       ...
                                       if($v['type']==0){//自购
                                         ...
                                         if(!empty($credit1_zg)){
                                             ...
                                             //$zgdd = pdo_fetch("select * from ".tablename("mc_credits_record")." where uniacid='{$_W['uniacid']}' and uid='{$uid}' and remark='自购订单返积分:{$v['orderid']}'"); 
												 $result =pdo_update ( $this->modulename . "_wphtjorder", array('sh'=>2,'yongjin'=>$tkorder['commission']), array ('id' => $v['id']));
												 if(!empty($result)){
                                       ...
                                       if($v['type']==0){//自购
                                          ...
                                          // $credit1_zg=$tkorder['xgyg']*$cfg['ejf']/100;  
                                           ...
                                           if(!empty($credit1_zg)){
                                               ...
                                               $result =pdo_update ( $this->modulename . "_wphtjorder", array('sh'=>2,'yongjin'=>$tkorder['xgyg']), array ('id' => $v['id']));
												if(!empty($result)){
            ...
            $order = pdo_fetchall("select * from ".tablename($this->modulename."_wphtjorder")." where weid='{$_W['uniacid']}' and openid='{$fans['openid']}' and sh=1 order by id desc");
        }elseif($op=='yf'){//已返
            $order = pdo_fetchall("select * from ".tablename($this->modulename."_wphtjorder")." where weid='{$_W['uniacid']}' and openid='{$fans['openid']}' and sh=2  order by id desc");
        }
        ...
        include $this->template ( 'user/wphorderlist' ); 