...
//             exit;
			 ...
			 $goods=cjsearch(1,$cfg['ptpid'],$tksign['sign'],$tksign['tbuid'],$_W,$cfg,urlencode($turl),2,'','','','',$tm,$px,$yhj);			 				 
			 if(empty($goods['total_results'])){
			 	$goods=cjsearch(1,$cfg['ptpid'],$tksign['sign'],$tksign['tbuid'],$_W,$cfg,urlencode($turl),2,'','','','',$tm,$px,$yhj);
			 }
			 $goods=$goods['result_list']['map_data'];
			 
			  $itemendprice=$goods['zk_final_price']-$goods['coupon_amount'];
			 $views['itemtitle']=$goods['title'];  //标题
			 $views['shoptype']=$goods['user_type'];  //1天猫
			 $views['itemid']=$goods['num_iid'];//商品ID
			 $views['itemprice']=$goods['zk_final_price'];//原价
			 $views['itemendprice']=$itemendprice;//折扣价-优惠券金额
			 $views['couponmoney']=$goods['coupon_amount'];//优惠券金额
			 $views['itemsale']=$goods['volume'];//月销量
			 $views['url']=$goods['item_url'];//商品链接
			 $views['shopTitle']=$goods['shop_title'];//店铺名称      
			 $views['sellernick']=$goods['shop_title'];//店铺名称    
			 $views['itempic']=$goods['pict_url'];//图片
			 $views['lm']=1;
			 $couponendtime=strtotime($goods['coupon_end_time']);
			 $ctime=date('Y/m/d H:i:s',$couponendtime);
			 // echo "<pre>";
			 // print_r($views);
			 // exit;
...
//           	exit;
             ...
             $views['tk_rate']=$res['commissionRate'];
               ...
               if(empty($views['itemprice']) || $views['itemprice']=='0.00'){
				 ...
				 if($_GPC['tlj']==1){
					 $_GPC['url']=$_GPC['tljurl'];
					 $views['url']=$_GPC['tljurl'];
					 $rhyurl=$_GPC['tljurl'];
				 }
