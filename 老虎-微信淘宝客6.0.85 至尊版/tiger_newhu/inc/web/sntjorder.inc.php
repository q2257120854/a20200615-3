				...
				if($op=="pljs"){//更新订单状态
							if(!$_GPC['id']){
								...
								foreach ($_GPC['id'] as $id){
										$row = pdo_fetch("SELECT * FROM " . tablename($this->modulename.'_sntjorder') . " WHERE id = :id", array(':id' => $id));
										...
										$tkrow = pdo_fetch("SELECT id,orderId,validCode FROM " . tablename($this->modulename.'_jdorder') . " WHERE orderId = :orderId", array(':orderId' => $row['orderid']));
										 ...
										 if($tkrow['validCode']==17 || $tkrow['validCode']==18){													 
											 $aupres=pdo_update($this->modulename."_sntjorder",array('sh'=>2), array('id' => $id));
											 if($aupres){
											 	  if($row['jltype']==1){//余额
													   $credit2_zg=$row['jl'];
														 if($row['type']==0){//自购
...
$page=$_GPC['page'];
 ...
 $cfg = $this->module['config'];
		...
		$psize = 20;
		$list = pdo_fetchall("select * from ".tablename($this->modulename."_sntjorder")." where weid='{$_W['uniacid']}' {$where} {$whereuid} order by id desc LIMIT " . ($pindex - 1) * $psize . ",{$psize}");
		$total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->modulename.'_sntjorder')." where weid='{$_W['uniacid']}' {$where} {$whereuid}");
		$pager = pagination($total, $pindex, $psize);
        include $this->template ( 'sntjorder' );