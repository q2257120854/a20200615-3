 ...
 global $_W,$_GPC;
		...
		if($operation=="rk"){
					...
					if(!empty($goods['coupon_info'])){
					 ...
					 $list['tkrates']=$goods['commission_rate']/100;  //佣金比例
					 $list['itemtitle']=$goods['title'];  //标题
					 $list['shoptype']=$goods['user_type'];  //1天猫
					 // $list['tm']=$goods['user_type'];  //1天猫
					 ...
					 $list['sellernick']=$goods['shop_title'];//店铺名称       
					 $list['userid']=$goods['seller_id'];//店铺ID  
					 $list['itempic']=$goods['pict_url'];//图片
					 $list['quan_id']=$goods['coupon_id'];//券ID
					 $list['couponstarttime']=strtotime($goods['coupon_start_time']);//券开始
					 $list['couponendtime']=strtotime($goods['coupon_end_time']);//券结束
					 $list['couponexplain']=$goods['coupon_info'];//券使用条件
					 $list['createtime']=TIMESTAMP;
					...
					$res=pdo_insert($this->modulename."_newtbgoods", $list);
					// echo "<pre>";
					// print_r($res);
					// exit;
					message('商品添加成功！', $this -> createWebUrl('tbgoods', array('op' => 'display')), 'success');
