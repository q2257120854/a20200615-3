...
class GoodRecordResponse {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['goodSnList'])){
				
				$this->goodSnList = $vals['goodSnList'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("goodSnList" == $schemeField){
				
				$needSkip = false;
				
				$this->goodSnList = array();
				$_size0 = 0;
				$input->readListBegin();
				while(true){
	...
	public function write($output){
		...
		if($this->goodSnList !== null) {
			
			$xfer += $output->writeFieldBegin('goodSnList');
			
			if (!is_array($this->goodSnList)){
				
				throw new \Osp\Exception\OspException('Bad type in structure.', \Osp\Exception\OspException::INVALID_DATA);
			}
			
			$output->writeListBegin();
			foreach ($this->goodSnList as $iter0){
