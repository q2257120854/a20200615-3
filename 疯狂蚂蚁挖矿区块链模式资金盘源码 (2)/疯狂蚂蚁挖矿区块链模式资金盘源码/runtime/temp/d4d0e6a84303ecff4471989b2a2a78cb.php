<?php /*a:1:{s:67:"/www/wwwroot/url.stak.cn/application/index/view/market/buying.html";i:1566132242;}*/ ?>
﻿<!DOCTYPE html>
<html>
...
<div id="pullrefresh" class="mui-content mui-scroll-wrapper">
    <div class="mui-scroll">
        <div id="segmentedControl" class="mui-segmented-control mui_title">
            ...
            <a class="mui-control-item mui-active">买进</a>
            <a class="mui-control-item" href="/market/selling.html">卖出</a>
        </div>

        <div id="item2" class="mui-control-content mui-active">
            <div class="mui_tiv">申请买入</div>
            <form class="mui-input-group mui_inp_rou">
                ...
                <div class="mui-input-row">
                    <label>单价</label>
                    <span><?php echo htmlentities(money($market['price'])); ?></span>
                </div>
            </form>
            <div class="mui_bot1">
                <a class="bt1  buy">确认申请</a>
            ...
            <div class="mui_jl">
                ...
                <span>
                    <a id="aprv" class="page" data-page="0">上一页</a>
                    <a id="anext" class="page" data-page="2">下一页</a>
                </span>
            </div>
            <div class="mui_online mui_push">
                <h3>买入记录列表</h3>
                ...
                <div class="mui-content-padded">
                    <div class="mui-content-padded">
                        <ul class="mui-pager">
                            <li>
                                <a id="bprv" class="page1" data-page="0">上一页</a>
                            </li>
                            <li>
                                <a id="bnext" class="page1" data-page="2">下一页</a>
                            </li>
                        </ul>
                      <br><br><br><br>
                    </div>
                ...
                success: function (e) {
                    if (e.code == 200) {
                        ...
                        if (e.data != '') {
                            ...
                            page = parseInt(page);
                            $('#aprv').attr('data-page', page - 1)
                            $('#anext').attr('data-page', page + 1)
                            $('#sss').html(html)
                        }
                ...
                success: function (e) {
                    if (e.code == 200) {
                        if (e.data != '') {
                            ...
                            page = parseInt(page);
                            $('#bprv').attr('data-page', page - 1)
                            $('#bnext').attr('data-page', page + 1)
                            $('#ccc').html(html)
                        }
                        if (e.data == ''  && page>0) {
                            layer.msg('已经到最后一页了');
...
<script>
    $('.buy').click(function () {
        var num = $('#select').val();
        layer.confirm('确定发布求购订单？', {icon: 3, title: '提示'}, function (index) {
            var load = layer.load(0);
            layer.close(index)
            $.ajax({
                url: "/trade/buy",
                ...
                data: {
                    number: num,
                    price: <?php echo htmlentities($market['price']); ?>
                }, success: function (e) {
                    layer.close(load)
                    if (e.code == 200) {
                        layer.msg('恭喜您、操作成功！', {time: 1000}, function () {
                            window.location.reload();
                        });
                    } else {
                        layer.msg(e.message)
