...
class Account extends Base
{
    ...
    public function index(Request $req)
    {
        ...
        $query = Db::table('account')->alias('a')
                    ...
                    ->field('p.avatar, p.idcard, p.nickname, a.create_at, a.username, a.type, a.status, a.authen, a.nolimit, a.inviter, w.money, w.deposit, w.score, w.score_deposit');
        // 条件：按类型搜索
        $type = $req->param('type');
	...
	public function nolimit(Request $req)
    {
        try {
            // 用户账号
            $username = $req->param('username');
            ...
            // 更新账号
          	if($req->param('type') == 1){
            	Db::table('account')->where('username', '=', $username)->update(['nolimit' => 1]);
            }else{
            	Db::table('account')->where('username', '=', $username)->update(['nolimit' => 0]);
            }
    ...
    public function capital(Request $req)
    {
        ...
        try {
			...
			// 更改蚁巢
            $poll= $req->param('poll/f');
            if ($poll != $user['wallet']['poll']) {
                $record[8] = [$user['wallet']['poll'], $poll - $user['wallet']['poll'], $poll];
            }
            // 更改蚁巢钱包
            $mpocket = $req->param('mpocket/f');
            if ($mpocket != $user['wallet']['mpocket']) {
                $record[7] = [$user['wallet']['mpocket'], $mpocket - $user['wallet']['mpocket'], $mpocket];
            }
            // 更改可售额度
            $cansell = $req->param('cansell/f');
            if ($cansell != $user['wallet']['cansell']) {
                $record[9] = [$user['wallet']['cansell'], $cansell - $user['wallet']['cansell'], $cansell];
            }
            if (!empty($record)) {
                // 更新钱包
                (new \app\index\controller\Wallet())->change($username, 88, $record);
                // 日志记录
                $this->log(11, $username, '原可用：' . $user['wallet']['money'] . '，现可用：' . $money . '，原冻结：' . $user['wallet']['deposit'] . '，现冻结：' . $deposit . '，原积分：' . $user['wallet']['score'] . '，现积分：' . $score . '，原冻结积分：' . $user['wallet']['score_deposit'] . '，现冻结积分：' . $score_deposit. '，原蚁巢：' . $user['wallet']['poll'] . '，现蚁巢：' . $poll. '，原蚁巢钱包：' . $user['wallet']['mpocket'] . '，现蚁巢钱包：' . $mpocket. '，原可售额度：' . $user['wallet']['cansell'] . '，现可售额度：' . $cansell);
            }
            // 提交事务
            Db::commit();
        } catch (\Exception $e) {
            ...
            if ($authen == 1) {
                ...
                if (!empty($authen_machine)) {
                  foreach ($authen_machine as $key => $product) {                            
                    (new Store())->give($product, $username,1);
                  }
                }
                (new \app\index\controller\Wallet())->change($username, 88, [
                  8 => [
                    $user['wallet']['poll'],
