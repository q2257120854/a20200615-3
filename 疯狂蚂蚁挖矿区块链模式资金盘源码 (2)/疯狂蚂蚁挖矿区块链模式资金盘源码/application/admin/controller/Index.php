...
class Index extends Base
{
    ...
    public function index()
    {
        ...
        // 今日产矿
        $today_clocks=Cache::get("today_clocks");
        if(!$today_clocks){
    ...
    public function setting(Request $req)
    {
        ...
        // 提交
        if ($req->isPost()) {
            $invit_money = $req->param('invit_money');
            // 保存设置
            try {
                Configure::set('hello.setting', ['invit_money' => $invit_money]);
            } catch (\Exception $e) {
                $this->error($e->getMessage());
                exit;
            }
            // 操作成功
            $this->success('恭喜您、操作成功！', '/admin/index/setting.html');
        }
        // 还没有内容
        if (empty($setting)) {
            $setting = [
